    ...
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body" id="f-username">
                <form action="<?= base_url('/' . bin2hex('guru/update/username')) ?>" method="post"
...
<script>
    function cek_uname_lama() {
        ...
        if (usern.length > 4) {
            $.ajax({
                url: "<?= base_url('/' . bin2hex('guru/cek-username-lama')); ?>",
                type: 'post',
                data: { uname: usern },
    ...
    function cek_uname() {
        ...
        if (usern.length > 4) {
            $.ajax({
                url: "<?= base_url('/' . bin2hex('guru/cek-username')); ?>",
                type: 'post',
                data: { uname: usern },
                success: function (result) {
                    let data = JSON.parse(result);
